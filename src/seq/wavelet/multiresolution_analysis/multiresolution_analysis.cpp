...
{
	static constexpr double forward[2][3] = {{sqrt_two, 0, 0}, {-sqrt_half, sqrt_two, -sqrt_half}};

	static constexpr std::ptrdiff_t backward_shift = 0;
	static constexpr double backward_c[2][2] = {{sqrt_half, 0}, {sqrt_half / 2, sqrt_half / 2}};
	static constexpr double backward_d[2][2] = {{0, 0}, {sqrt_half, 0}};
};

struct Daubechies_wavelet
{
private:
	static constexpr auto c0 = (1 + sqrt_three) / (4 * sqrt_two);
	static constexpr auto c1 = (3 + sqrt_three) / (4 * sqrt_two);
	static constexpr auto c2 = (3 - sqrt_three) / (4 * sqrt_two);
	static constexpr auto c3 = (1 - sqrt_three) / (4 * sqrt_two);
	...
	const double 	   eps2)
{
	...
	const auto fn = [](double x) {
		return std::exp(-x) * std::sin(4 * pi * x) + x * (1 - x) * std::sin(16 * pi * x);
	};
