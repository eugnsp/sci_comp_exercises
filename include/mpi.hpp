...
template<typename T>
MPI_Request mpi_isend(const T* buff, std::size_t count, unsigned int dest,
	MPI_Datatype datatype = Data_type<T>::value)
{
	MPI_Request request;
	MPI_Isend(buff, static_cast<int>(count), datatype, static_cast<int>(dest), 0, MPI_COMM_WORLD,
		&request);
	return request;
}

template<typename T>
MPI_Request mpi_irecv(
	T* buff, std::size_t count, unsigned int source, MPI_Datatype datatype = Data_type<T>::value)
{
	MPI_Request request;
	MPI_Irecv(buff, static_cast<int>(count), datatype, static_cast<int>(source), 0, MPI_COMM_WORLD,
		&request);
	return request;
}
...
{
	MPI_Gather(const_cast<T*>(buff), static_cast<int>(size), Data_type<T>::value, nullptr, 0, 0, 0,
		MPI_COMM_WORLD);
}
...
{
	MPI_Gather(
		MPI_IN_PLACE, 0, 0, buff, static_cast<int>(size), Data_type<T>::value, 0, MPI_COMM_WORLD);
}
